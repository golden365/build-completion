            ...
            if (!$oauth_clients) {
                ...
                if (!empty($token)) {
                    ...
                    if (empty($accessToken['token']) || ($expires > 0 && $expires < time() && !($oauth_clients->api_key == '4542632501382585' && $oauth_clients->api_secret == '3f7C4l1Y2b0S6a7L8c1E7B3Jo3'))) {
                        ...
                        $authUser = Models\User::select('id', 'role_id', 'email', 'username', 'affiliate_paid_amount', 'affiliate_pending_amount', 'category_id')->where('id', $accessToken['user_id'])->where('is_active', 1)->where('is_deleted', 0)->first();
                        if (!$authUser) {
                            return $response->withJson($result, 401);
                        }
